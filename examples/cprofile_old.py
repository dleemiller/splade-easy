"""
Enhanced Performance profiling for SPLADE-Easy (old SpladeIndex API) with cProfile
"""
...
class OldAPIProfiler:
    """Performance profiler for old SpladeIndex API"""
    ...
    def profile_indexing(self, docs: List[str], index_dir: str, 
        ...
        for batch_idx in range(0, len(docs), batch_size):
            ...
            # Time encoding separately (estimate)
            encode_start = time.perf_counter()
            # Pre-encode the batch to measure encoding time
            ...
            if (batch_idx + batch_size) % 500 == 0:
                print(f"    Indexed {batch_idx + batch_size} docs, "
    ...
    def profile_search(self, index_dir: str, num_queries: int = 100,
                      warmup_queries: int = 10, mode: str = "disk") -> Dict[str, Any]:
        """Profile search performance with warmup and detailed timing"""
        print(f"\nProfiling {num_queries} search queries in {mode} mode (with {warmup_queries} warmup)...")
        ...
        retriever = SpladeIndex.retriever(index_dir, mode=mode)
        
        # Warmup phase
        print("  Running warmup queries...")
        for i in range(warmup_queries):
            query = queries[i % len(queries)]
            _ = retriever.search_text(query, model=self.model, top_k=10)
        ...
        for i in range(num_queries):
            ...
            # Measure retrieval time (includes re-encoding, but we can subtract)
            retrieval_start = time.perf_counter()
            results = retriever.search_text(query, model=self.model, top_k=10, return_text=True)
            retrieval_time = time.perf_counter() - retrieval_start
            # Approximate actual retrieval time by subtracting encoding
    ...
    def profile_memory_vs_disk(self, index_dir: str, num_queries: int = 50,
        ...
        queries = [
            ...
            "neural networks", "software engineering", "cloud computing"
        ]
        ...
        for i in range(warmup_queries):
            query = queries[i % len(queries)]
            _ = retriever_disk.search_text(query, model=self.model, top_k=5)
        ...
        for i in range(warmup_queries):
            query = queries[i % len(queries)]
            _ = retriever_memory.search_text(query, model=self.model, top_k=5)
...
def main():
    ...
    profiler = OldAPIProfiler(model_name)
    ...
    # Profile indexing
    if use_cprofile:
        indexing_results = profiler.run_with_cprofile(
            profiler.profile_indexing, docs, index_dir
        )
    else:
    ...
    if use_cprofile:
        search_disk_results = profiler.run_with_cprofile(
            profiler.profile_search, index_dir, num_queries=100, mode="disk"
        )
    else:
        search_disk_results = profiler.profile_search(index_dir, num_queries=100, mode="disk")
    
    print("\n" + "="*60)
    print("SEARCH RESULTS (DISK MODE)")
    print("="*60)
    print(f"Queries/sec: {search_disk_results['queries_per_sec']:.1f}")
    print(f"Total results: {search_disk_results['total_results']}")
    print(f"\n{format_timing_stats(search_disk_results['search_stats'], 'Total search times')}")
    
    # Profile search - memory mode
    if use_cprofile:
        search_memory_results = profiler.run_with_cprofile(
            profiler.profile_search, index_dir, num_queries=100, mode="memory"
        )
    else:
        search_memory_results = profiler.profile_search(index_dir, num_queries=100, mode="memory")
    
    print("\n" + "="*60)
    print("SEARCH RESULTS (MEMORY MODE)")
    print("="*60)
    print(f"Queries/sec: {search_memory_results['queries_per_sec']:.1f}")
    print(f"Total results: {search_memory_results['total_results']}")
    print(f"\n{format_timing_stats(search_memory_results['search_stats'], 'Total search times')}")
    
    # Profile memory vs disk comparison
