...
def generate_synthetic_docs(num_docs: int) -> list[str]:
    ...
    for i in range(num_docs):
        ...
        doc = f"Document {i}: This is about {topic}. " * 3  # Match old API
        docs.append(doc)
...
def profile_search(index_dir: str, model, num_queries: int = 100) -> dict:
    ...
    for i in range(num_queries):
        ...
        start_time = time.time()
        results = reader.search_text(query, model=model, top_k=10, return_text=True)
...
def profile_memory_vs_disk(index_dir: str, model, num_queries: int = 50):
    ...
    for i in range(num_queries):
        query = queries[i % len(queries)]
        start_time = time.time()
        results = reader_disk.search_text(query, model=model, top_k=5)
    ...
    for i in range(num_queries):
        query = queries[i % len(queries)]
        start_time = time.time()
        results = reader_memory.search_text(query, model=model, top_k=5)
